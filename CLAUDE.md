...
**Current Status:** M0 closed; M1 (central-path allocator) under way. The public API runs over the real central-path allocator: classify → central free lists / extent-backed large path, with genuine `free`/`realloc`/`malloc_usable_size`, errno semantics, C23 sized frees, the extended `topo_*x` API, opt-in C++ operators, and the Rust `GlobalAlloc` adapter — identical over POSIX and the seLe4n simulator (G-sim). The full §25 realloc state machine (W15) is complete: move with failure-preserves-the-original (allocate-before-free, arena preserved), same-class/within-extent in-place grow, and in-place **shrink** that splits off and returns a medium/large allocation's tail pages to the backend across a page boundary (best-effort under always-correct semantics; cache-served/exhausted-split keep the allocation whole), plus aligned-allocation validation and calloc multiply+rounding overflow guards with full-usable zeroing. Capability-backed arenas (W9) ride on top: a live multi-arena data path with the full §22/§36.4/§36.13 lifecycle (create/delegate/reset/destroy/revocation), per-arena isolation, quota/authority/label enforcement, NUMA policy modes, and a C arena API (`topo_arena_create/delegate/reset/destroy`). Extent hooks & custom backing (W10) ride on the same seam: the §23.2 `ExtentHooks` interface + `HookProvider` adapter run the whole central path over a user-supplied backing, with §23.3 contracts enforced and the §23.4 conditional-correctness assumption modeled in Lean. The hugepage-aware backend (W11) rides on the §18.6 region-cache seam: the four §19.2 components (HugeAllocator / HugeCache / HugePageFiller / RegionCache) as a real, backend-agnostic placement subsystem over the provider seam — nine §19.4 occupancy bins (each hugepage in exactly one, H-003), packing-scored placement (§19.3), partial subrelease guarded by H-005, §19.7 coverage metrics in stats, and the §19.8 H-001..H-005 invariants checked in debug — wired into the live large path through the existing `RegionCacheHook`, identical over POSIX and the seLe4n simulator. The release controller (W12), live NUMA topology router (W13), and the lifetime/hotness/site-profile **placement policy** (W14) — fed live by a **minimal, off-by-default heap-sampling slice** (W17-3: lock-free per-thread Poisson decision, alloc-free `libc::backtrace` capture, right-censored sampled-object lifecycle, all behind a re-entrancy guard) — ride on top, the placement policy upholding the §24.5 safety boundary (it changes locality, never size/alignment/validity) by construction. Security & hardening (W18) rides on top too, behind granular profile-composed features (`crate::harden`): junk filling (§29.6 — fill-on-alloc/free + a sound use-after-free verify-on-reuse canary for **small *and* large** objects, the large path carrying a per-extent canary provenance so a stale bit can never false-abort a correct program), a delayed-reuse quarantine accounted separately as `quarantine.bytes` (§29.4 — budgets, drain, background convergence, `topomalloc_quarantine_*` control, off by default, with windowless double-free detection via an authoritative per-object quarantined state — a second per-object bitmap drained free-bit-first under the span lock, immune to address reuse), **randomized, tightly right-aligned** sampled guarded allocations with a real `mprotect(PROT_NONE)` trap (§29.5 — the new `TopoBackingProvider::protect` seam + `LargeAllocator::allocate_guarded`), and scrub-before-downgrade (§36.12 — a lock-free per-arena `label` + a non-PUBLIC arena's backing scrubbed before recycle, the runtime image of the Lean `scrub_before_downgrade` theorem) — all opt-in so `performance` pays nothing, with out-of-line large metadata + generation/integrity tags (W18-1) and double/invalid-free detection (W18-2) always-on. Debugging & sanitization modes (W19) ride on top: the Appendix-B invariant checklist as first-class runtime code (`crate::debug` — one total, side-effect-free callable per group B.1–B.5, run as `debug_assert!`s under `debug-checks` and as CI test oracles, gating `performance` to zero), including the §30.2 **pagemap↔descriptor** check (`PageMap::check_invariants` — every owned page names a descriptor whose range covers it, catching a stale-after-recycle entry the descriptor-level oracles cannot) and the **redzone** sweep (`verify_free_patterns` — every central-free object reads as `FREE_PATTERN` under junk-fill), both wired into the engine oracle and exercised in CI under `debug-checks` over the integration suite; ASan/MSan/**LeakSanitizer** integration beside the existing TSan (the RSEQ asm self-disabling under sanitizers it cannot instrument, `cfg(topo_sanitize_no_asm)`; LSan on the real-allocator C-ABI path, by-design monotonic metadata in `xtask/lsan-suppressions.txt`); and a deterministic test mode (`crate::deterministic` — seeded randomness, force-slow-path/force-purge, monotonic trace IDs) with a self-consistency replay proving a captured §33.7 trace replays byte-identically under the same seed (the Lean executable-model differential remains W21-2b). Front-end caches (M2) and the remaining M1 pieces land per the plan.
...
cargo xtask test [--kind unit|prop|diff|fuzz|loom|tsan|asan|msan|rseq]
...
`reserved`, distinct from advised-away `released`), `quarantine.bytes` (the W18-3 held bytes, separately
accounted — `0` when the quarantine is off), and `arena.destroyed` (a real cumulative `ArenaTable::destroyed_count`, bumped at each
...
- Sanitizers (W19-2, §30.3): `cargo xtask test --kind tsan|asan|msan` — ThreadSanitizer over
  the W6/W7 concurrency + W18 hardening, **AddressSanitizer / MemorySanitizer** over the
  `topo-core` library (default + hardened), and **LeakSanitizer** on the real-allocator
  C-ABI pass (`detect_leaks=1`; by-design monotonic metadata in `xtask/lsan-suppressions.txt`);
  the RSEQ asm disables itself under ASan/MSan (`cfg(topo_sanitize_no_asm)`), so there are no
  asm false positives
- Appendix-B invariant checks (W19-1, DD-2): one callable checker per group (B.1 global /
  B.2 cache / B.3 span / B.4 hugepage / B.5 arena) gathered in `crate::debug`, incl. the
  §30.2 **pagemap↔descriptor** check (`PageMap::check_invariants`) and the **redzone**
  free-pattern sweep (`verify_free_patterns`, under junk-fill). The cheap per-span B.3 check
  runs as a `debug_assert!` at **every central transition**; the O(state) sweeps (B.1
  reachability + the pagemap walk, full B.2 distinctness, B.5 incl. the hook-install +
  stale-generation guards, the redzone sweep) run on demand from `Allocator::check_invariants`
  (the C `topomalloc_debug_check_now`) and as the G-core/G-conc/G-mem/G-arena oracles —
  exercised in CI under `debug-checks` over the `topo-core` unit tests *and* the cross-crate
  integration suite (`topo-tests --features debug-checks`) — each with a **negative** test
  proving it catches a real violation; the B.2 layer also fuzzed (`debug_checks`)
- Deterministic mode (W19-3, §30.4): `crate::deterministic` (seeded randomness via
  ...
  with a **self-consistency replay** (a captured seeded §33.7 trace replays byte-identically
  under the same seed, and differs under a different seed)
