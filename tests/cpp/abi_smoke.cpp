...
int main() {
    ...
    {
        static_assert(sizeof(topomalloc_stats_t) == 27u * sizeof(uint64_t),
        ...
        assert(st.peak_live_bytes >= st.live_bytes);
        assert(topomalloc_stats_snapshot(&st, (uint64_t) 1 << 40) == -1); // strict validation
        char json[8192];
        ...
        assert(std::strstr(explain, "RSS is ") != nullptr);
    }
