...
| `TopoMalloc/ArenaLifecycle.lean` | the §22.3/§36.13 arena lifecycle state machine + `canTransition` (pinned 1:1 to Rust `ArenaState`); `only_draining_reaches_destroyed`, `quarantine_only_retries_draining`, `allowsAlloc_iff_active` | W9-2/W9-6 |
...
| `TopoMalloc/Exec.lean` | executable model (arena-attributed live set) + §33.7 **text-grammar** trace replay incl. the W9 `ARENA_RESET`/`ARENA_DESTROY` events; flags injected violations | W1-10 |
| `Check.lean` | `lake exe check`: the G-table gate, the trace-oracle gate, and the pagemap (W3-3d), §36.6 provider (`providerChainGate`), §20.1 extent-state (`extentStateGate`, W4-2d) and §22.3 arena-lifecycle (`arenaLifecycleGate`, W9-2) differentials | — |
