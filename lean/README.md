...
| `TopoMalloc/HugePageFiller.lean` | the §19.4 `classifyBin` (nine bins, H-003 by construction); H-002 occupancy-is-sum; the filler as a per-page state machine (place/free/subrelease) with H-001/H-004/H-005 preservation; intra-hugepage placement disjointness (`place_preserves_outside`/`place_preserves_existing_live`/`place_live_is_disjoint_union`: a placement only flips its own range and partitions the live set); H-005 over the `Range` geometry; `hugeBinGate` decidable checks (pins Rust `classify_bin`) | W11 |
...
| `Check.lean` | `lake exe check`: the G-table gate, the trace-oracle gate, and the pagemap (W3-3d), §36.6 provider (`providerChainGate`), §20.1 extent-state (`extentStateGate`, W4-2d), §23.3 hook-contract (`hookContractGate`, W10) and §19.4 hugepage-bin (`hugeBinGate`, W11) differentials | — |
