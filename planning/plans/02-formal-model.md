...
> **▸ Implementation status.** The **full** W1 set is **landed** in [`lean/`](../../lean/) and gated in CI
> (`lake build` proof-checks every module; `lake exe check` runs the G-table, coverage, and trace-oracle
> gates). All §33.4 theorems, all §36.17 bridge families (single-core **and** the SMP/multicore extensions),
> the §9.4/§9.5 proofs for the real tuned 32 KiB size-class table (W1-4e, the runtime's table), the coupled
> TopoMalloc↔seLe4n simulation, and the §33.7 text-grammar trace oracle (W1-10) are complete. W1-14 SMP is
> **proved** (not staged) by interleaving semantics over all schedules. No `sorry`s and no `native_decide`;
> the only trusted axioms are the four §33.5 RSEQ primitives. A "proved vs. deliberately abstracted" register
> and the module map are in [`lean/README.md`](../../lean/README.md).
